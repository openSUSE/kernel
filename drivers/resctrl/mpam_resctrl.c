...
static int *__alloc_mbwu_array(struct mpam_class *class, u16 num_mbwu_mon)
{
	...
	int *array __free(kvfree) = kvmalloc_array(num_mbwu_mon, sizeof(*array), GFP_KERNEL);
	if (!array)
		return ERR_PTR(-ENOMEM);
...
static int mpam_resctrl_monitor_init_abmc(struct mpam_resctrl_mon *mon)
{
	...
	int *rmid_array __free(kvfree) = kvmalloc_array(num_rmid, sizeof(*rmid_array), GFP_KERNEL);
	if (!rmid_array) {
