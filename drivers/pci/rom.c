				 ...
				 bool expect_valid)
{
	...
	if (check_add_overflow((unsigned long)image, PCI_ROM_HEADER_SIZE - 1UL,
				&header_end))
		return false;
				      ...
				      void __iomem *rom, size_t size)
{
	...
	if (check_add_overflow((unsigned long)pds, PCI_ROM_DATA_STRUCT_LEN + 1UL,
				&end))
		return false;
	...
	if (check_add_overflow((unsigned long)pds, data_len - 1UL, &end))
		return false;
