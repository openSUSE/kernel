...
{
	size_t pref_ring_size = 0;

	if (vmbus_devs[channel->device_id].dev_type == HV_UNKNOWN)
		pref_ring_size = 0x11000U;
	else if (vmbus_devs[channel->device_id].dev_type == HV_FCOPY)
		pref_ring_size = 0x4000U;

	return pref_ring_size;
}
