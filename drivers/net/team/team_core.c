			      ...
			      const struct team_mode *new_mode)
{
	...
	team->ops.port_disabled = new_mode->ops->port_disabled;
			     ...
			     unsigned char *haddr)
{
	...
	if (port) {
		...
		if (port_ops && port_ops->parse) {
			if (port_ops->parse == parse_header_kabi_helper)
				ret = port_ops->parse2(skb, port->dev, haddr);
			else
