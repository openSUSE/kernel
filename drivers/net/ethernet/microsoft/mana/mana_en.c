...
static int mana_get_gso_hs(struct sk_buff *skb)
{
	...
	if (skb->encapsulation) {
		gso_hs = skb_inner_transport_offset(skb) +
		...
		if (skb_shinfo(skb)->gso_type & SKB_GSO_UDP_L4) {
			...
			gso_hs = skb_transport_offset(skb) + tcp_hdrlen(skb);
		}
	}
...
int mana_create_eq(struct mana_port_context *apc)
{
	...
	if (WARN_ON(apc->eqs))
		return -EEXIST;
	apc->eqs = kcalloc(gc->max_num_queues, sizeof(struct mana_eq),
			  GFP_KERNEL);
	if (!apc->eqs)
		return -ENOMEM;
...
int mana_probe(struct gdma_dev *gd, bool resuming)
{
	...
	if (!resuming) {
		...
		for (i = 0; i < ac->num_ports; i++) {
			...
			schedule_work(&apc->queue_reset_work);
...
void mana_remove(struct gdma_dev *gd, bool suspending)
{
	...
	for (i = 0; i < ac->num_ports; i++) {
		...
		apc = netdev_priv(ndev);
		cancel_work_sync(&apc->queue_reset_work);
