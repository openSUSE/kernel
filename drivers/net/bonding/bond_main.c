...
static int bond_ipsec_add_sa(struct xfrm_state *xs)
{
	...
	dev_hold_track(real_dev, &tracker, GFP_ATOMIC);
	rcu_read_unlock();
	if (!real_dev) {
		err = -ENODEV;
		goto out;
	...
	err = real_dev->xfrmdev_ops->xdo_dev_state_add(xs);
	if (!err) {
		ipsec->xs = xs;
...
out:
	dev_put_track(real_dev, &tracker);
	return err;
}
...
static void bond_ipsec_add_sa_all(struct bonding *bond)
{
	...
	list_for_each_entry(ipsec, &bond->ipsec_list, list) {
		...
		if (real_dev->xfrmdev_ops->xdo_dev_state_add(ipsec->xs)) {
			     ...
			     unsigned char *haddr)
{
	...
	if (slave) {
		...
		if (slave_ops && slave_ops->parse) {
			if (slave_ops->parse == parse_header_kabi_helper)
				ret = slave_ops->parse2(skb, slave->dev, haddr);
			else
