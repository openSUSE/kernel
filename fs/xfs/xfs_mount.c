	...
	unsigned long long	new_max_bytes)
{
	...
	const xfs_extlen_t	max_write = xfs_calc_atomic_write_max(mp);
	const xfs_extlen_t	max_group = mp->m_groups[XG_TYPE_AG].blocks;
	const xfs_extlen_t	max_group_write = xfs_calc_perag_awu_max(mp);
	int			error;
 ...
 "max atomic write size of %lluk cannot be larger than max allocation group write size %lluk",
	...
	if (new_max_fsbs == 1) {
		if (mp->m_ddev_targp->bt_bdev_awu_max ||
		    (mp->m_rtdev_targp && mp->m_rtdev_targp->bt_bdev_awu_max)) {
		} else {
			xfs_warn(mp,
